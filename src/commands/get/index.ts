...
`,
  ...
  static flags = {
    steamId: Flags.string({char: 'i', description: 'Steam ID to fetch games for.', required: true})
  }
  ...
  private _getServiceFromArgument(serviceName: string, flags: any): GameFetcher {
      switch(true) {
          case serviceName.toLowerCase() === 'steam':
              return new SteamService(axios, process.env.STEAM_API_KEY || "", flags.steamId);
          default:
            return {getOwnedGames: () => of([]), authorize: () => of("")};
      }
  }
}
